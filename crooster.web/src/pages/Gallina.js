...
export default function Gallina() {
    ...
    const {placa, apodo, color, selected, img, items, arete, madre, padre, amigo} = state;
    ...
    const handleChange = (e) => {
        switch (e.target.name) {
            case 'placa':
                ...
                break;
                case 'arete':
                dispatch({type: ACTIONS.ARETE, payload: e.target.value});
                break;
                case 'madre':
    ...
    return (
        <div>
         <Form buttons={BUTTONS} handleSubmit={handleSubmit} title="Gallina">
         <div className="input-field col s6">
          <input required placeholder="Placa" name="placa" type="text" 
          className="validate" id="placa" onChange={handleChange} value={placa} />
        </div>
        <div className="input-field col s6">
        ...
        <div className="input-field col s12">
          <input required placeholder="Arete" name="arete" type="text" 
          className="validate" onChange={handleChange} value={arete} />
        </div>
